...
namespace sh::render
{
	...
	public:
		...
		SH_RENDER_API auto GetAllShaderPass() const -> const std::vector<LightingPassData>& { return passDatas; }

		SH_RENDER_API auto GetProperties() const -> const std::unordered_map<std::string, PropertyInfo>& { return properties; }
		SH_RENDER_API auto GetProperty(const std::string& name) const -> const PropertyInfo*;

		SH_RENDER_API auto GetShaderAST() const -> const ShaderAST::ShaderNode& { return shaderNode; }

		SH_RENDER_API auto IsUsingLight() const -> bool { return bUsingLight; }
		SH_RENDER_API auto IsUsingSkin() const -> bool { return bUsingSkin; }
	private:
		void Clear();
