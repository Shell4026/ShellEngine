...
namespace sh::render
{
	...
	public:
		...
		SH_RENDER_API void SetOrthographic(bool bEnable);
		SH_RENDER_API void SetCustomProjectionMatrix(const glm::mat4* matrix);
		...
		SH_RENDER_API auto GetPos(core::ThreadType thr) const -> const glm::vec3& { return pos[thr]; }
		SH_RENDER_API auto GetLookPos(core::ThreadType thr) const -> const glm::vec3& { return to[thr]; }
		SH_RENDER_API auto GetUpVector(core::ThreadType thr) const -> const glm::vec3& { return up[thr]; }
		SH_RENDER_API auto GetFovRadian(core::ThreadType thr) const -> float { return fovRadians[thr]; }
		SH_RENDER_API auto GetNearPlane(core::ThreadType thr) const -> float { return nearPlane[thr]; }
		SH_RENDER_API auto GetFarPlane(core::ThreadType thr) const -> float { return farPlane[thr]; }
		SH_RENDER_API auto GetWidth(core::ThreadType thr) const -> float { return width[thr]; }
		SH_RENDER_API auto GetHeight(core::ThreadType thr) const -> float { return height[thr]; }

		SH_RENDER_API auto IsOrtho() const -> bool { return !bPerspective; }
	protected:
		SH_RENDER_API void SyncDirty() override;
		SH_RENDER_API void Sync() override;
		/// @brief dirtyMask |= 256; 후 SyncDirty호출 필요
	...
	protected:
		...
		core::SyncArray<float> fovRadians = { 0.0f, 0.f };
		core::SyncArray<float> nearPlane = { 0.1f, 0.1f };
		core::SyncArray<float> farPlane = { 1000.f, 1000.f };
		core::SyncArray<float> width = { 1.f, 1.f };
		core::SyncArray<float> height = { 1.f, 1.f };
