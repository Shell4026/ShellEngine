...
namespace sh::render
{
	...
	protected:
		...
		int id;
	protected:
		ShaderPass(int id, ShaderType type);
	...
	public:
		...
		SH_RENDER_API auto operator==(const ShaderPass& other) -> bool;
		...
		SH_RENDER_API auto GetUniformBinding(const std::string& name) const -> std::optional<uint32_t>;
		...
		template<typename T>
		void AddUniformArray(const std::string& name, std::size_t size, UniformType type, uint32_t binding, ShaderStage stage);
	...
	template<typename T>
	inline void ShaderPass::AddUniform(const std::string& name, UniformType type, uint32_t binding, ShaderStage stage)
	{
	...
	/// @brief 배열 타입의 유니폼을 추가하는 함수. 반드시 셰이더의 유니폼과 같은 순서로 호출하여 넣을 것.
	/// @brief size == 1이면 AddUniform()을 호출 한 것과 같다.
	/// @tparam T 타입
	/// @param name 이름
	...
	/// @param stage 셰이더 스테이지
	template<typename T>
	inline void ShaderPass::AddUniformArray(const std::string& name, std::size_t count, UniformType type, uint32_t binding, ShaderStage stage)
	{
		if (count == 1)
		{
			AddUniform<T>(name, type, binding, stage);
			return;
		}
		std::size_t align = 16; // 배열의 경우 무조건 16바이트 정렬
		core::reflection::TypeInfo* typeinfo = &core::reflection::GetType<T>();
	...
	template<>
	inline void ShaderPass::AddUniform<Texture>(const std::string& name, UniformType type, uint32_t binding, ShaderStage stage)
	{
